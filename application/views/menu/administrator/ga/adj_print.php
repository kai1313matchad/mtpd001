...
<body>
    ...
    <div class="container con-prime">
        <div class="row row-bd-bottom">
            <div class="col-xs-12 text-center">
                <h2><strong>FORM PENYESUAIAN BARANG MATCHAD</strong></h2>
            </div>
        </div>
        <div class="row row-bd-bottom">
    ...
    <script>        
        ...
        {
            $.ajax({
                url : "<?php echo site_url('administrator/Genaff/ajax_pick_adj/')?>" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data)
                {
                    $('[name="pr-adj-code"]').text(data.ADJGA_CODE);
                    $('[name="pr-adj-info"]').text(data.ADJGA_INFO);
                    $('[name="pr-adj-date"]').text(moment(data.ADJGA_DATE).format('DD-MMM-YYYY'));
                    var $tr = $('<tr>').append(                            
                            $('<td class="text-center">'+data.GD_CODE+'</td>'),
                            $('<td class="text-center">'+data.GD_NAME+'</td>'),
                            $('<td class="text-center">'+data.ADJGA_OLDQTY+' '+data.GD_MEASURE+'</td>'),
                            $('<td class="text-center">'+data.ADJGA_CURQTY+' '+data.GD_MEASURE+'</td>')
                            ).appendTo('#tb_content');
                },
