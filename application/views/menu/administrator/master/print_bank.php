    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-sm-12 col-xs-12 table-responsive">
                <table id="dtb_master_bank" class="table table-bordered" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            <th>Nama Bank</th>
                            <th>No. Rek.</th>
                ...
                success: function(data)
                {
                    for (var i = 0; i < data.length; i++)
                    {
                        var tr = $('<tr>').append(
                            ...
                            $('<td>'+data[i]["BANK_ACC"]+'</td>'),
                            $('<td>'+data[i]["BANK_ACCNAME"]+'</td>'),
                            $('<td>'+data[i]["BANK_PRODTYPE"]+'</td>'),
                            $('<td>'+data[i]["BANK_BRANCH"]+'</td>'),
                            $('<td>'+data[i]["BANK_CURR"]+'</td>'),
                            $('<td>'+data[i]["COA_ACC"]+'</td>'),
                            $('<td>'+data[i]["BANK_INFO"]+'</td>')
                            ).appendTo('#tb_content')                        
                    }
                    dtable();
                },
