<!-- Page Content -->
    ...
    <script>
        ...
        {            
            //Ajax Load data from ajax
            $.ajax({
                url : "<?php echo site_url('administrator/Finance/ajax_pick_gmdet/')?>/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data)
                {                       
                    ...
                    for (var i = 0; i < data.length; i++) {
                            var gir_id = data[i]["GIR_ID"];
                            pick_giroin_record(gir_id);
                            var cust_id = $('[name="cust_id"]').val();
                            var cust_name = $('[name="giro_custname"]').val();

                            var $tr = $('<tr>').append(
                            $('<td>').text(cust_id + '\xa0' + cust_name),    
