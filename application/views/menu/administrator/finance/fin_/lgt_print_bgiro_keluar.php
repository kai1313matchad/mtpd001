<!-- Page Content -->
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Cetak Laporan Pencairan Giro Keluar</h1>
                    </div>
                </div>
                <div class="row">
                ...
                <div class="row" id="printArea">
                    ...
                    <div class="text-center">
                        <h3><strong><u>LAPORAN Pencairan Giro Keluar</u></strong></h3>
                                ...
                                <div class="panel-body">
                                    <div class="table-responsive">
                                        ...
                                        <table id="tb_gm" class="table table-condensed">
                                            <thead>
                                                <tr>
                                                    <th class="col-sm-2 col-xs-2 text-center">No. Pencairan</th>
                                                    ...
                                                    <th class="col-sm-2 col-xs-2 text-center">No. Bank</th> 
                                                    <th class="col-sm-2 col-xs-2 text-center">Customer</th> 
    ...
    <script>
        ...
        function printPre()
        {
            ...
            window.open ( "<?php echo site_url('administrator/Finance/pageprint_giro_out/')?>"+seg1+'/'+seg2,'_blank');
        }
        ...
        {            
            ...
            $.ajax({
                url : "<?php echo site_url('administrator/Finance/show_giro_keluar/')?>/",
                type: "POST",
                ...
                {                    
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if ((tgl_cair != "") && (tgl_cair != data[i]["CAIR_DATE"])){
                             var $st = $('<tr>').append(
                                      $('<td>').text(''),
                                      $('<td>').text(''),
                                      $('<td>').text(''),
                                      $('<td>').text(''),
                                      $('<td>').css({'text-align':'right','font-weight':'bold'}).text('Total'+' '+formattanggal(tgl_cair)+' :'),
                                      $('<td>').css({'border-top':'2px solid','text-align':'right','font-weight':'bold'}).text(formatCurrency(ttgl,".",",",2))
                                      ).appendTo('#tb_content');
                             // tgl_cair = data[i]["CAIR_DATE"];
                             ttgl = 0;
                        }
                        if ((bank !="") && (bank != data[i]["BANK_ID"])) {
                            var $st2 = $('<tr>').append(
                                      $('<td>').text(''),
                                      $('<td>').text(''),
                                      $('<td>').text(''),
                                      ...
                                      $('<td>').css({'border-top':'2px solid','text-align':'right','font-weight':'bold','border-bottom':'double'}).text(formatCurrency(tbank,".",",",2))
                                      ).appendTo('#tb_content');

                                    tbank = 0;
                        }
                        if ((cabang !="") && (cabang != data[i]["BRANCH_NAME"])) {
                            var $st3 = $('<tr>').append(
                                      $('<td>').text(''),
                                      $('<td>').text(''),
                                      $('<td>').text(''),
                                      $('<td>').text(''),
                                      $('<td>').text(''),
                                      $('<td>').css({'border-top':'double','text-align':'right','font-weight':'bold','border-bottom':'double'}).text(formatCurrency(total,".",",",2))
                                      ).appendTo('#tb_content');
                        ...
                        if (cabang != data[i]["BRANCH_NAME"])  {
                            var $tr = $('<tr>').append(
                                      $('<td>').css({'font-weight':'bold'}).text(data[i]["BRANCH_NAME"])
                                    ).appendTo('#tb_content');
                        ...
                        if (tgl_cair != data[i]["CAIR_DATE"]) {
                            var $tr = $('<tr>').append(
                                      $('<td>').css({'font-weight':'bold'}).text(formattanggal(data[i]["CAIR_DATE"]))
                                      ).appendTo('#tb_content');
                            tgl_cair = data[i]["CAIR_DATE"];
                        }
                        ...
                        gkm = jenis.substring(0,2);
                            ttgl = ttgl + (data[i]["GR_AMOUNT"] * 1);
                            tbank = tbank + (data[i]["GR_AMOUNT"] * 1);
                            total = total + (data[i]["GR_AMOUNT"] * 1);
                            var $tr = $('<tr>').append(
                                      $('<td>').text(data[i]["GR_CODE"]),
                                      $('<td>').text(data[i]["GR_NUMBER"]),
                                      $('<td>').text(data[i]["GR_DATE"]), 
